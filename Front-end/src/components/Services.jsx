      ...
      <div id="services-section" className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
          {serviceList.map((service, idx) => (
            <div key={idx} className="premium-hover-card rounded-2xl bg-slate-50 border border-slate-100 overflow-hidden text-left flex flex-col">
              <div className="h-44 w-full bg-slate-200">
                <img className="h-full w-full object-cover" src={service.image} alt={`${service.title} service`} />
              </div>
              <div className="p-5 flex-grow flex flex-col justify-between">
                <div>
                  <h3 className="text-base font-extrabold text-blue-950">{service.title}</h3>
                ...
                <div className="mt-5 flex justify-end">
                  <button 
                    onClick={onBookClick}
                    className="flex h-9 w-9 items-center justify-center rounded-full bg-teal-500 text-white hover:bg-teal-600 shadow-sm transition active:scale-95"
                  >
